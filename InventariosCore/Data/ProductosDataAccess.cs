    ...
    public class ProductosDataAccess
    {
        ...
        public List<Producto> ObtenerProductos(bool soloActivos = true)
        {
            ...
            try
            {
                ...
                // Conexión y ejecución como en otros métodos
                _dbAccess.Connect();
                ...
                foreach (DataRow row in resultado.Rows)
                {
                    ...
                    producto.Costo = Convert.ToDecimal(row["costo_unitario"]);
                    producto.Stock = row["stock"] == DBNull.Value ? (int?)null : Convert.ToInt32(row["stock"]);
                    producto.Ubicacion = row["ubicacion"].ToString() ?? "";
                    producto.Clave = row["clave"].ToString() ?? "";
                    producto.Estatus = Convert.ToInt32(row["estatus"]);
                    producto.AplicaImpuesto = row["aplica_impuesto"] != DBNull.Value && Convert.ToBoolean(row["aplica_impuesto"]);
                    producto.IdImpuesto = row["id_impuesto"] == DBNull.Value ? (int?)null : Convert.ToInt32(row["id_impuesto"]);

                    productos.Add(producto);
                }


                _logger.Info($"Productos encontrados: {productos.Count}");
                return productos;
            }
            catch (Exception ex)
            {
                _logger.Error(ex, "Error al obtener productos");
                return productos; // vacío en caso de error
            }
            finally
            {
                _dbAccess.Disconnect();
